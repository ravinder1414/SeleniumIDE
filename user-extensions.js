...
Selenium.prototype.doEnsureChecked = function(cbLocator) {
	
	var element = this.page().findElement(cbLocator)
	Selenium.prototype.doEcho(element.checked);
    	if (element.checked){}
	else
	{
		selenium.doClick(cbLocator);
	}
        return;
}
Selenium.prototype.doVerifyInTable = function(cbLocator, substr) {
	
	var element = this.page().findElement(cbLocator)
	var res = substr.split(",");
	var findmetext = res[0];
	var col = res[1];
	var yes = 0;
	Selenium.prototype.doEcho(findmetext);
	Selenium.prototype.doEcho(col);
	var table = this.page().findElement(cbLocator)
	//var table = this.browserbot.findElement(cbLocator);
	for(var i=0;i<table.rows.length;i++)
	{ 
		if(getText(table.rows[i].cells[col])==findmetext)
		{	
			yes=1;
			break;
		}		
	...
	if(yes==1)
	{	
		Selenium.prototype.doEcho(findmetext + " Found in Table");
		
	}
	else
	{
		Selenium.prototype.doEcho(findmetext + " Not Found in Table");
		Selenium.prototype.doEcho("Raising Unexpected Exception to Fail the command");
		Selenium.prototype.doEch()
	}
					
        return;
}
Selenium.prototype.doQuad2ChkContMgrActivityddHome=function(str){
var i, AllActivities, len, spltfrst3;
AllActivities = str.split(",");
len=AllActivities.length;
for(i=1;i<len;i++){
 spltfrst3= AllActivities[i].substr(0,3);
 if (spltfrst3.indexOf("AD") == -1 )
 { if (spltfrst3.indexOf("ME") == -1)
  {
   if (spltfrst3.indexOf("CS") == -1)
   { break;
   }
  }
 }  
...
{
storedVars['res']=false;
return;
}

}

Selenium.prototype.doQuad2ChkContMgrActivityddAddActivity=function(str){
var i, AllActivities, len, spltfrst3;
AllActivities = str.split(",");
len=AllActivities.length;
for(i=1;i<len;i++){
...
Selenium.prototype.doStoreMonthName = function(month, name){
//var name;
//name = ${month}==0?"Jan":${month}==1?"Feb":${month}==2?"Mar":${month}==3?"Apr":${month}==4?"May":${month}==5?"Jun":${month}==6?"Jul":${month}==7?"Aug":${month}==8?"Sep":${month}==9?"Oct":${month}==10?"Nov":"Dec";
storedVars['name'] = month==1?"Jan":month==2?"Feb":month==3?"Mar":month==4?"Apr":month==5?"May":month==6?"Jun":month==7?"Jul":month==8?"Aug":month==9?"Sep":month==10?"Oct":month==11?"Nov":"Dec";
